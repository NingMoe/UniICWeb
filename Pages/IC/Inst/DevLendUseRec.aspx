<%@ Page Language="C#" MasterPageFile="~/Templates/Sub.master" AutoEventWireup="true" CodeFile="DevLendUseRec.aspx.cs" Inherits="Sub_Course" %>

<%@ Register Src="~/Modules/PageCtrl.ascx" TagPrefix="uc1" TagName="PageCtrl" %>

<asp:Content ID="Content1" ContentPlaceHolderID="Content" runat="Server">
    <form id="formAdvOpts" runat="server">
        <h2 style="margin-top: 10px; font-weight: bold"><%=ConfigConst.GCSysKindLend %>使用记录</h2>
        <input type="hidden" id="szGetKey" name="szGetKey" />   
        <div class="toolbar">
            <div class="tb_info">
                <div class="UniTab" id="tabl">
                   <a href="DevLendList.aspx" id="DevLendList"><%=ConfigConst.GCSysKindLend %>使用状况</a>                
                <a href="DevLendUseRec.aspx" id="DevLendUseRec"><%=ConfigConst.GCSysKindLend %>使用记录</a>
                <!-- <a href="DevPCResvRec.aspx" id="DevPCResvRec"><%=ConfigConst.GCSysKindLend %>预约记录</a>-->
                </div>
            </div>
        </div>
       <div>
                ...
                $("#btnExport").button().click(function () {
                    var vDateStart = $("#<%=dwStartDate.ClientID%>").val();
                     ...
                     $.lhdialog({
                         ...
                         lock: true,
                         data: Dlg_Callback,
                         content: 'url:Dlg/outPortdevlenduserec.aspx?op=set&dwPID=' + vLogonName + '&dwDevKind=' + dwDevKind + '&szGetKey=' + vRoomid + '&startdate=' + vDateStart + '&enddate=' + vDateEnd
                     });
                 });
            });           
            $("#btnOK").button();

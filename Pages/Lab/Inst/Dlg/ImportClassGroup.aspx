<%@ Page Language="C#" MasterPageFile="~/Templates/Dlg.master" AutoEventWireup="true" CodeFile="ImportClassGroup.aspx.cs" Inherits="_Default" %>
...
<%if(pagedata.dwStep == 0){ %>
        <div id="Step1">
            ...
            <br /><hr />
            <p>导入文件例子(<a href="../../downloadfile/classGroup.csv">点击下载</a>):</p>
            <%=m_szOut %>
        </div>
<%} else if(pagedata.dwStep == 2){ %>
        <div id="Step2">
            <p class="header ui-widget-header"><span class="StepName">选择对应学期：</span><select id="dwDeadLine" name="dwDeadLine"><%=szYearTerm %></select></p>


            <p class="header ui-widget-header"><span class="StepName">第二步：确认字段格式</span><span class="msgtip" name="szMessage"></span></p>
            <div class="result"><p>共解析到<span name='dwTotalLine'></span>条数据,<label style="color:red">学工号首位为0的请设置单元格格式为文本</label></p></div>
            ...
            <div class="result">
                <p>【<span name="szFileName"></span>】总<span name='dwTotalLine'></span>条数据，其中<span class="Imported" name="dwImported"></span>条导入成功 , <span class="Failed" name="dwFailed"></span>条导入失败。</p>
                 ...
                 <p>失败数据：
                    ...
                    <!--<div class="ErrLines" name="szErrLines"></div>。</p>-->
            <!--    <p><a href="<%=pagedata.szErrListFile %>">下载导入失败的数据</a>-->
                     </p>

...
public partial class searchAccount : UniPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (m_Request.Device.Get(vrGet, out vtDev) == REQUESTCODE.EXECUTE_SUCCESS && vtDev != null)
        {
            ...
            {
                szOut += "{\"id\":\"" + vtDev[i].dwDevID + "\",\"label\": \"" + vtDev[i].szDevName + "(" + vtDev[i].szAssertSN+","+vtDev[i].szRoomName+ ")" + "\",\"dwRoomID\": \"" + vtDev[i].dwRoomID + "\",\"dwLabID\": \"" + vtDev[i].dwLabID + "\",\"dwKindID\": \"" + vtDev[i].dwKindID + "\"}";
                if (i < vtDev.Length - 1)
                {
                    szOut += ",";
                }

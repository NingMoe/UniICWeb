    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       // vrGet.szGetID = szTerm;
        ...
        if (m_Request.Device.Get(vrGet, out vtDev) == REQUESTCODE.EXECUTE_SUCCESS && vtDev != null)
        {
            ...
            {
                szOut += "{\"id\":\"" + vtDev[i].dwDevID + "\",\"label\": \"" + vtDev[i].szDevName + "(" + vtDev[i].szModel+","+vtDev[i].szRoomName + ")" + "\",\"szDevSN\": \"" + vtDev[i].dwDevSN + "\",\"szAssertSN\": \"" + vtDev[i].szAssertSN + "\",\"szRoom\": \"" + vtDev[i].szRoomName + "\",\"szDevKind\": \"" + vtDev[i].szKindName + "\"}";
                if (i < vtDev.Length - 1)
                {
                    szOut += ",";
                }

...
$("table").delegate(".devCtrlPowerup", "click", function () {
        ...
        $("table").on("click", ".lnkAssertRoom", function () {
            $.lhdialog({
                title: '<%=ConfigConst.GCRoomName %>',
                width: '650px',
                height: '500px',
                lock: true,
                data: Dlg_Callback,
                content: 'url:<%=MyVPath %>Pages/<%=ConfigConst.GCSysFrame %>/sys/Dlg/NewRoom.aspx?op=set&roomid='+$(this).data('id')+'&msg=' + escape($(this).text())
            });
        });      

        $("table").on("click", ".lnkAssertDevice", function () {
            $.lhdialog({
                ...
                lock: true,
                data: Dlg_Callback,
                content: 'url:<%=MyVPath %>Pages/<%=ConfigConst.GCSysFrame %>/sys/Dlg/newAssert.aspx?op=set&id='+$(this).data('id')+'&msg=' + escape($(this).text())
            });
        });   

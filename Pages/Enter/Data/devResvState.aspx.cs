...
public partial class Resv_searchCls :UniPage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string szStartLine = Request["startLine"];
        if (szNeedLine != null && szNeedLine != "")
        {
            uNeedLine = Parse(szNeedLine);
        }
        if (szStartLine != null && szStartLine != "")
        {
            uStartLine = Parse(szStartLine);
        }
       ...
       // vrGet.dwGetType = 0;
        ...
        vrGet.szReqExtInfo.dwNeedLines = uNeedLine;
        vrGet.szReqExtInfo.dwStartLine = uStartLine;
        ...
        if (m_Request.Device.GetDevResvStat(vrGet, out vtRes) == REQUESTCODE.EXECUTE_SUCCESS && vtRes != null && vtRes.Length > 0)
        {
            ...
            for (int i = 0; i < vtRes.Length; i++)
            {
                ...
                if (resvInfo != null && resvInfo.Length > 0)
                {
                    for (int j = 0; j < resvInfo.Length; j++)
                    {
                        ...
                        szResvInfo = szResvInfo + "{" + "\"name\":\"" + devNameTemp + "\"," + "\"devID\":" + devIDTemp + "," + "\"value\":" + uResvTime.ToString() + "}";
                        //if (i < vtRes.Length-1)
                        {
                            szResvInfo += ",";
                        }
            ...
            szDevInfo += "]";
            szResvInfo += "]";
            string sztotal = vrGet.szReqExtInfo.dwTotolLines.ToString();
            if (sztotal == null || sztotal == "")
            {
                sztotal = "0";
            }
          
            szOut = "{" + "\"resvInfo\":" + szResvInfo + ",\"DevList\":" + szDevInfo + ",\"OpenTime\":" + szOpenTime + ",\"startLine\":" + vrGet.szReqExtInfo.dwStartLine.ToString() + ",\"totalLine\":" + sztotal + "}";

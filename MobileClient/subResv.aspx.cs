...
public partial class _Default : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (resvSubmit!=null&&resvSubmit=="ok"&&!IsPostBack)
        {
            ...
            szStartTime = (UintParse(szStartTime) / 100 + ":" + UintParse(szStartTime) % 100);
            string szEndTime = Request["dwEndTime"];
            szEndTime = (UintParse(szEndTime) / 100 + ":" + UintParse(szEndTime) % 100);
            ...
            AddGroupMember(group.dwGroupID, accno.dwAccNo, (uint)GROUPMEMBER.DWKIND.MEMBERKIND_PERSONAL,accno.szTrueName);
            ...
            for (int i = 0; i < szAccnoList.Length; i++)
            {
                if (szAccnoList[i] != "")
                {
                    ...
                    {
                        AddGroupMember(group.dwGroupID, accinfo.dwAccNo, (uint)GROUPMEMBER.DWKIND.MEMBERKIND_PERSONAL,accinfo.szTrueName);
                    }
                }
            }

            setValue.dwMemberKind = (uint)UNIRESERVE.DWMEMBERKIND.MEMBERKIND_GROUP;
            setValue.dwPurpose = (uint)UNIRESERVE.DWPURPOSE.USEFOR_PERSONNAL+(uint)UNIRESERVE.DWPURPOSE.USEFOR_STUDYROOM;
            ...
            setValue.dwMemberID = group.dwGroupID;
            setValue.szMemberName = accno.szTrueName + "预约组";
    ...
    public uint UintParse(string s)
    {
        uint ret = 0;
     ...
     //   vrGet.dwGetType = (uint)DEVRESVSTATREQ.DWGETTYPE.DEVRESVSTAT_ALL;
        ...
        if (m_Request.Device.GetDevResvStat(vrGet, out vtRes) == REQUESTCODE.EXECUTE_SUCCESS && vtRes != null&&vtRes.Length>0)
        {
            ...
            //if (uDevID != 0)
            {
                ...
                uMinUser = (uint)vtRes[0].dwMaxUsers;

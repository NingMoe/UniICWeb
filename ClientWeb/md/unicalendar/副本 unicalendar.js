...
/*
*外部依赖说明：
*2014/9/29后版本需使用2014/9/29后pro内uni.compareDate方法
*2014/9/29前版本请使用：
    uni.compareDate=function(date1, date2, s) {
        if (typeof (date1) == "string") date1 = uni.parseDate(date1);
        if (typeof (date2) == "string") date2 = uni.parseDate(date2);
        var d1 = date1.getTime()/1000/60;
        var d2 = date2.getTime() / 1000 / 60;
        if (s == "m") {
            d1 = parseInt(d1);
            d2 = parseInt(d2);
        }
        else if (s == "h") {
            d1 = parseInt(d1 / 60) * 60;
            d2 = parseInt(d2 / 60) * 60;
        }
        else {// day
            d1 = parseInt(d1 / 1440) * 1440;
            d2 = parseInt(d2 / 1440) * 1440;
        }
        return d1-d2;
    }
*/
(function ($, uni) {
    var defaults = {
        mode: 'd',
        modes: "dwm",
        style: 'dft',//dft：默认 cld： 纯日历  mini：简化版 alone：独立版（只显示时间）
        operate: 'click',
        ui: false,
        allowHistory:false,//是否允许查看历史
        width: 700,
        cellHeight: 50,
        pctrl: undefined,//列表分页显示 对象 属性分别num trigger triggerHeight （月模式不支持）
        objTitleMinWidth: 76,
        borderWidth: 1,
        shedule: false,//日程表
        relative: false,//相对时间
    ...
    var funs = {
        ...
        stateSet: stateSet,
        //刷新
        uploadCld: function (date, mode, options) {
            if (date) idate = date;
    ...
    $.fn.uniCalendar = function (options) {
        ...
        if (opt.beginDay)
            ...
            idate = new Date();
        initView();
        redLine = $("<div class='cld-red-line' style='position:absolute;top:-1000px;left:-1000px;'><div class='title'>00:00</div><div class='arrw'></div></div>");
        cld.after(redLine);
        return funs;
    };
    ...
    function initView() {
        ...
        if (opt.style == "alone") opt.objTitleMinWidth = 0;//独立版不显示对象
    ...
    function initTop(top) {
        ...
        tr.append("<td class='cld-top-c'></td>");
    ...
    .html("<div class='cld-bt-date'><span class='cld-todate-txt'>" + (opt.shedule ? opt.titleText.weekorder : opt.buttonText.today) + "</span></div>");
        if (!opt.shedule) todate.click(clickToday);
        htd.append(todate);
    ...
    function monthView(con) {
        ...
        htd.append(prevb);
        var todate = $("<td class='cld-h-cell cld-h-bt cld-h-bt-today'/>")
    .html("<div class='cld-bt-date'><span class='cld-todate-txt'>" + (opt.shedule ? opt.titleText.monthorder : opt.buttonText.today) + "</span></div>");
        if (!opt.shedule) todate.click(clickToday);
        htd.append(todate);
        var nextb = $("<td class='cld-h-cell cld-h-bt cld-h-bt-next'/>")
    .html("<div class='cld-bt-next'><span class='cld-next-txt'>" + opt.buttonText.next + "</span></div>");
        ...
        if (opt.shedule) week.append("<td class='cld-wtd cld-wk-sn' style='width:" + (cw - bw) + "px;border-width:" + bw + "px;'><span class='cld-week-txt'>"
    ...
    function checkDate() {
        if (opt.shedule && opt.dateStart && opt.dateEnd) {
    ...
    function dayTime() {
        ...
        if (!opt.allowHistory && uni.compareDate(tm_date, new Date()) < 0) {
        ...
        var qzTbl = $("<table class='cld-sch-tbl' date='" + dt + "' wwd='" + wwd + "' secnum='" + opt.secnum + "' style='width:auto;border-collapse:collapse;'/>");
        //课表视图
        if (opt.shedule) {
            $(".cld-todate-txt", iqz).html(parseInt(wwd / 10) + opt.titleText.week);
            if (opt.height) {
                qzs.css({ overflow: "auto", height: opt.height + "px" });
            }
            ...
            for (var j = startL; j < endL; j++) {
                ...
                if ((!opt.relative && uni.compareDate(op_start, idate) > 0) || (op_end && (uni.compareDate(idate, op_end) > 0)))//比较日期
                    continue;//开放时区外
                ////
                ...
                var objQz = $("<div class='cld-obj-qz " + odd_even + "' objId='" + o[j].id + "' objName='" + o[j].title + "' style='min-height:" + opt.cellHeight + "px;position:relative;'/>").appendTo(qzs);
                var objTbl = $("<table class='cld-obj-tbl' style='width:100%;border-collapse:collapse;'/>").appendTo(objQz);
    ...
    function redLineHandler(data) {
        return function (e) {
            ...
            if (data.bar_end) bar_time = data.bar_end;
                ...
                bar_time = m2approx(data.h * 60 + Math.ceil(e.offsetX / m_w * 60 * dayOpt.snipTime));
    ...
    function monthTime() {
        ...
        tblcell.each(function (i) {
            ...
            var dtitle = "<div class='cld-m-cell-title'>" + date.getDate() + "</div>";
            var dinfo = "<div class='cld-m-cell-info' style='min-height:" + opt.cellHeight + "px;'/>";
            cell.html(dtitle);
            cell.append(dinfo);
            if (opt.evSelTime) {
                cell.css("cursor", "pointer");
                cell.addClass("cld-op-in");
                cell.unbind("click");
    ...
    function modePlan() {
        ...
        var pctrl = opt.pctrl;
        if (pctrl && opt.mode != "m") {
            iqz.pctrl($(".cld-pctrl", iqz), pctrl.num || 20, {
                type: "dropdown",
                item: ".cld-obj-qz",
                trigger: pctrl.trigger || "drop",
                triggerHeight: pctrl.triggerHeight || 120
            });
        }
        if (opt.evFinishDraw) opt.evFinishDraw(idate.format("yyyy-MM-dd"), idate, opt, iqz);
    }
    //日视图
    function dayPlan() {
        var m = iqz.find(".cld-body");
        m.find(".cld-plan-bar").remove();
        var qzs = opt.shedule ? m.find(".cld-sec-qz") : m.find(".cld-obj-qz");
        var ps = opt.plans;
        qzs.each(function () {
            var qz = $(this);
    ...
    function monthPlan() {
        ...
        cells.each(function (i, _cell) {
                ...
                if (date < today) {
                    ...
                    for (var j = 0; j < plans.length; j++) {
                        ...
                        if (uni.compareDate(date, start) >= 0 && uni.compareDate(date, end, "m") <= 0) {
                            ...
                            var rsvd = "";//预约详细
                            if (plan.allDay) {
                                var rsvd = formatDateS(start) + "至" + formatDateS(end)
                            }
                            else {
                                var rsvd = formatTime(start) + "至" + formatTime(end);
                            }
                            title += " | " + (plan.title || "") + ":[" + rsvd + "]";
                            if (plan.allDay) {
            ...
            if (nofree) {
                ...
                cell.unbind("click");
                cell.addClass("cld-cell-nofree").parent("td").addClass("cld-ttd-nofree");
            }
            else {
                cell.removeClass("cld-cell-nofree").removeClass("cld-ttd-nofree");
            }
            if (opt.style == "mini")
    ...
    function actTch() {//合并单节次为实验 以第一节为标识 diff为节次差
        var tchs = stateSet.tchs;
        tchs.sort();
        var len = tchs.length,
            i = 0,
        ...
        while (i < len) {
            ...
            var key = "" + tc;
            if (stateSet.testDic[key]) {//更新或存入实验
        ...
        if (opt.evRefState) opt.evRefState(stateSet.tests, stateSet.testDic);
    }
    function refCld() {

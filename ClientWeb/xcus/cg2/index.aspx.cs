...
public partial class ClientWeb_xcus_all_index : UniClientPage
{
    ...
    private void GetNewResv()
    {
        ...
        req.dwCheckStat = (uint)ADMINCHECK.DWCHECKSTAT.CHECKSTAT_ADMINOK;
        ...
        if (m_Request.Reserve.GetReserveForShow(req, out rlt) == REQUESTCODE.EXECUTE_SUCCESS)
        {
            ...
            {
                RESVSHOW rsv=rlt[i];
                if ((rsv.dwProperty & (uint)UNIRESERVE.DWPROPERTY.RESVPROP_UNOPEN) > 0) continue;//不公开
                if ((rsv.dwStatus & (uint)ADMINCHECK.DWCHECKSTAT.CHECKSTAT_ADMINOK) == 0) continue;//未审核
                if ((rsv.dwStatus & (uint)UNIRESERVE.DWSTATUS.RESVSTAT_DONE) > 0) continue;//已过期
    ...
    private string GetNotice()
    {
        ...
        if (list != null && list.Length > 0)
        {
            for (int i = 0; i < list.Length; i++)
            {
                ...
                infoContent += "<div class='msg_box'><div class='img-circle msg_img'><span class='glyphicon glyphicon-volume-up'></span></div><div><strong class='color1'>" + title + "</strong><div class='con songti'>"

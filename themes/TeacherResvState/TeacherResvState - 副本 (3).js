$.fn.TeachingResvState = function (options) {
    ...
    $(this).each(function () {
        ...
        var vClientwidht = (document.body.clientWidth);//当前屏幕分辨率
        ...
        $.ajax({
            ...
            success: function (data, textStatus) {
               ...
               */
                ...
                var vthcolpan = (parseInt(endTime / 100) - parseInt(startTime / 100)) - 1;
                var vthWeek = $("<th colspan='" + vthcolpan + "' style='font-size:12px' class='resvWeeks'></th>");
                var vdvthhaedwidth = parseInt((vthcolpan * thWidth) / 7) - 30;
              //  if (vClientwidht<1280)
                {
                    vdvthhaedwidth = 110;
                    //vdvthhaedwidth=70;
                }
             //   alert(vdvthhaedwidth);
                //  alert(parseInt((vthcolpan * thWidth) / 7) - 3);
                //将星期改成div模式原来是th模式
                for (var i = 1; i <= 7; i++) {
                   ...
                   //alert((parseInt(endTime / 100) - parseInt(startTime / 100)));
                   var vthHeadText = "星期" + Week[dateNow.getDay()] + "</br>" + dateNow.getFullYear() + "-" + (dateNow.getMonth() + 1) + "-" + dateNow.getDate();
                   var vDivthHead = $("<div style='margin:5px;height:33px;font-size:8px;display:inline-block;width:"+(vdvthhaedwidth)+"px' id=" + (id + 0) + ">" + vthHeadText + "</div>");
                   vDivthHead.click(function () {
                    ...
                    if (vSplitDay==0) {
                        vDivthHead.addClass("ui-widget-selectResv ");
                    }
                    else {
                        vDivthHead.addClass("ui-widget-headerResv ");
                    }
                    vthWeek.append(vDivthHead);
                ...
                var vtdheadwidth = 150;//房间名称所在单元的长度大小
                    ...
                    if (vResvInfo != null && vResvInfo.length > 0)
                    {
                        ...
                        for (var i = 0; vResvInfo != null && i < vResvInfo.length; i++) {
                            ...
                            */
                            ...
                            vdiv.css("width", (vdivWidth - vcolumSqur*2*2 )+ "px");
                            ...
                            debugger;
                            var left = parseInt((vTetmpLeft-vOpenLeft+parseInt(opentime / 10000) % 100 / 60) * thWidth) + thWidth -(4+2 * vcolum);
                            left = left + (vtdheadwidth - thWidth);
                ...
                for (var DevI = 0; DevI < devList.length; DevI++) {
                    var devName = "";
                    var devNameFull = devList[DevI].devName;
                    if (devList[DevI].devNameExt != null && devList[DevI].devNameExt != "") {
                        devName = devList[DevI].devNameExt;
                    }
                    else {
                        devName=devList[DevI].devName;
                    }
                    
                    var devID = devList[DevI].devID;
                    var tbodyWeekTimetr = $("<tr ></tr>");
                    tbodyWeekTimetr.append($("<th title='" + devNameFull + "' devid=" + devID + ">" + devName + "</th>").css("width", vtdheadwidth + "px"));
